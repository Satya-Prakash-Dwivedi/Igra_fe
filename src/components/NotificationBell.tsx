...
const NotificationBell: React.FC<NotificationBellProps> = ({
  ...
  const getIcon = (type: string) => {
    switch (type) {
      ...
      default:
        return <Bell size={14} className="text-text-dim" />
    }
  }
    ...
    <div className="relative" ref={dropdownRef}>
        ...
        className={cn(
          ...
          variant === 'sidebar'
            ? isCollapsed
              ...
              : 'w-full px-4 py-2.5 bg-white/[0.02] hover:bg-white/[0.05] border border-white/5'
      ...
      >
        <div className="relative">
          <Bell
            ...
            className={cn(
              ...
              isOpen || unreadCount > 0 ? 'text-primary' : 'text-text-dim group-hover:text-white'
            )}
          />
          {unreadCount > 0 && (
            <span
        ...
        {variant === 'sidebar' && !isCollapsed && (
          <span className="ml-4 font-bold text-[11px] uppercase tracking-wider text-text-dim group-hover:text-white flex-1 text-left">
            Notifications
          </span>
        )}
      </button>
      ...
      {isOpen && (
        <div
          className={cn(
            ...
            variant === 'sidebar' ? (isCollapsed ? 'left-14 top-0' : 'left-0') : 'right-0'
          )}
        >
          {/* Header */}
          <div className="px-5 py-4 border-b border-white/5 flex items-center justify-between bg-white/[0.02]">
            <div className="flex items-center gap-2">
              ...
              <h3 className="text-white font-bold text-[11px] uppercase tracking-[0.1em]">
                Notifications
              </h3>
            </div>
            {unreadCount > 0 && (
              ...
              >
                <Check size={10} className="group-hover:scale-110 transition-transform" /> Mark all
                read
              </button>
            )}
          </div>
          ...
          <div className="max-h-[320px] overflow-y-auto custom-scrollbar py-1">
            {notifications.length === 0 ? (
              <div className="py-12 flex flex-col items-center justify-center gap-3 opacity-30">
                ...
                <p className="text-[9px] font-bold uppercase tracking-[0.2em] text-text-dim">
                  No new alerts
                </p>
              </div>
            ) : (
