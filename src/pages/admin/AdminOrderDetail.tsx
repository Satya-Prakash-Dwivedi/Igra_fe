        ...
        <div key={i} className="relative group">
          ...
          <div className="mb-4">
            <div className="flex flex-col md:flex-row md:items-center justify-between gap-2 mb-2">
              <span className="text-base font-bold text-white tracking-tight ">{event.type.replace(/_/g, ' ').toLowerCase().replace(/\b\w/g, c => c.toUpperCase())}</span>
          ...
          {event.type === 'REVISION_REQUESTED' && event.data ? (
            <div className="bg-white/[0.02] p-4 rounded-lg border border-white/5 shadow-inner space-y-4 max-w-2xl">
              {(event.data as any).notes && (
                <div className="space-y-1">
                  <span className="text-xs text-text-dim/40 font-bold uppercase tracking-wide block">Notes</span>
              ...
              {(event.data as any).assets && Array.isArray((event.data as any).assets) && ((event.data as any).assets).length > 0 && (
                <div className="space-y-2 mt-4">
                  <span className="text-xs text-primary/60 font-bold uppercase tracking-wide block">Attached Reference Files</span>
                  <div className="grid grid-cols-1 sm:grid-cols-2 gap-3 mt-2">
                    {((event.data as any).assets as any[]).map((asset: any) => (
                      <div key={asset._id} className="flex items-center justify-between bg-black/40 border border-primary/10 rounded-lg p-3 text-xs">
            ...
            <div className="bg-success/5 p-4 rounded-lg border border-success/20 shadow-inner space-y-4 max-w-2xl">
              <div className="space-y-1">
                <span className="text-xs text-success/60 font-bold uppercase tracking-wide block">Status</span>
...
const ItemCard: React.FC<{
  ...
  return (
    <div className={cn(
      ...
      isExpanded ? "border-primary/20 shadow-lg shadow-primary/5" : "border-white/5 hover:border-white/10"
    )}>
      <div className="p-5 flex flex-col md:flex-row md:items-center justify-between gap-3 cursor-pointer relative z-10" onClick={onToggle}>
        <div className="flex items-center gap-3">
          ...
          <div>
            ...
            <div className="flex flex-wrap items-center gap-3">
              ...
              <span className="text-xs font-bold uppercase tracking-wide text-text-dim/40">{item.creditsQuoted} credits</span>
              <span className="w-1 h-1 rounded-full bg-white/10" />
              <span className="text-xs font-bold uppercase tracking-wide text-text-dim/40">Revision {item.usedRevisions} of {item.allowedRevisions}</span>
            </div>
          </div>
        </div>

        <div className="flex items-center gap-3">
          <StatusBadge status={item.status} className="px-4 py-1.5 rounded-lg text-xs font-bold uppercase tracking-wide" />
          <div className={cn(
            ...
            <div className="space-y-5">
              <section className="space-y-6">
                <div className="flex items-center gap-3">
                   ...
                   <h4 className="text-xs font-bold text-text-dim/40 uppercase tracking-wide">Workflow protocol</h4>
                </div>
                ...
                {(isLoading || error) && (
                  <div className="flex items-center gap-3">
                    {isLoading && <p className="text-xs font-bold uppercase tracking-wide text-primary animate-pulse">Synchronizing status...</p>}
                    {error && <p className="text-error text-xs font-bold uppercase tracking-wide">{error}</p>}
                  </div>
                )}
              </section>
              ...
              <section className="space-y-6">
                <div className="flex items-center gap-3">
                   ...
                   <h4 className="text-xs font-bold text-text-dim/40 uppercase tracking-wide">Configuration parameters</h4>
                </div>
                <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                  {Object.entries(item.params).map(([key, val]) => {
                    if (key === 'showLinkInput') return null;
                    return (
                      <div key={key} className="p-5 bg-black/20 rounded-lg border border-white/5 hover:border-white/10 transition-colors">
                        <p className="text-xs text-text-dim/20 uppercase font-bold tracking-wide mb-1.5">{key.replace(/_/g, ' ').replace(/([A-Z])/g, ' $1').trim()}</p>
                        {Array.isArray(val) ? (
              ...
              <div className="space-y-5">
                ...
                {item.usedRevisions > 0 && latestRevisionEvent && (
                  <section className="space-y-6">
                    <div className="flex items-center gap-3">
                      ...
                      <h4 className="text-xs font-bold text-primary uppercase tracking-wide">Latest Revision Request</h4>
                    </div>
                    <div className="bg-primary/5 p-4 rounded-lg border border-primary/20 space-y-4 shadow-xl">
                      {(latestRevisionEvent.data as any).notes && (
                        <div className="space-y-1">
                          <span className="text-xs text-primary/60 font-bold uppercase tracking-wide block">Notes</span>
                ...
                <section className="space-y-6">
                  <div className="flex items-center gap-3">
                    ...
                    <h4 className="text-xs font-bold text-text-dim/40 uppercase tracking-wide">Output deliverables ({(item.assets ?? []).filter(a => a.role === 'OUTPUT').length + (item.deliveryLinks?.length || 0)})</h4>
                  </div>
                  {((item.assets ?? []).filter(a => a.role === 'OUTPUT').length > 0 || (item.deliveryLinks?.length || 0) > 0) ? (
                    <div className="grid grid-cols-2 gap-4">
                      {(item.assets ?? []).filter(a => a.role === 'OUTPUT').map((asset) => (
                        <div key={asset._id} className="group/asset relative bg-primary/5 border border-primary/10 rounded-lg overflow-hidden hover:border-primary/40 transition-all duration-500 shadow-xl">
                          <div className="aspect-[4/3] w-full bg-black/60 flex items-center justify-center relative overflow-hidden">
                            ...
                            <div className="absolute inset-0 bg-black/80 opacity-0 group-hover/asset:opacity-100 transition-all duration-300 flex items-center justify-center gap-3 backdrop-blur-md">
                              <button onClick={() => handleDownload((asset as any).url, asset.originalName)} className="w-8 h-8 flex items-center justify-center bg-white/5 hover:bg-white text-text-dim hover:text-black rounded-lg transition-all shadow-xl">
                                 <Download size={18} />
                              </button>
                              {asset.mimeType.startsWith('image/') && (
                                 <button onClick={() => onPreview(asset)} className="w-8 h-8 flex items-center justify-center bg-primary text-white rounded-lg transition-all shadow-xl shadow-primary/20 scale-110">
                                    <Maximize2 size={18} />
                                 </button>
                              )}
                              <button onClick={() => handleRemoveAsset(asset._id)} className="w-8 h-8 flex items-center justify-center bg-error/10 text-error rounded-lg hover:bg-error hover:text-white transition-all shadow-xl shadow-error/10">
                                 <Trash2 size={18} />
                              </button>
                            </div>
                          </div>
                          <div className="p-4 bg-white/[0.02]">
                            <p className="text-xs text-white font-bold truncate tracking-tight">{asset.originalName}</p>
                      ...
                      {item.deliveryLinks?.map((link, idx) => (
                        <div key={idx} className="group/link relative bg-primary/5 border border-primary/10 rounded-lg overflow-hidden hover:border-primary/40 transition-all duration-500 shadow-xl">
                          <div className="aspect-[4/3] w-full bg-black/60 flex items-center justify-center relative overflow-hidden p-4 text-center">
                            <div className="space-y-3">
                              ...
                              <p className="text-xs font-bold text-white uppercase tracking-wide truncate max-w-full">{link}</p>
                            </div>
                            <div className="absolute inset-0 bg-black/80 opacity-0 group-hover/link:opacity-100 transition-all duration-300 flex items-center justify-center gap-3 backdrop-blur-md">
                              <a href={link} target="_blank" rel="noreferrer" className="w-8 h-8 flex items-center justify-center bg-primary text-white rounded-lg transition-all shadow-xl shadow-primary/20 scale-110">
                                <ExternalLink size={18} />
                              </a>
                              <button onClick={() => handleRemoveLink(link)} className="w-8 h-8 flex items-center justify-center bg-error/10 text-error rounded-lg hover:bg-error hover:text-white transition-all shadow-xl shadow-error/10">
                                <Trash2 size={18} />
                              </button>
                            </div>
                          </div>
                          <div className="p-4 bg-white/[0.02]">
                            <p className="text-xs text-white font-bold truncate tracking-tight">External Delivery Link</p>
                            <div className="flex items-center justify-between opacity-40 text-[10px] font-bold uppercase tracking-wide mt-1.5">
                              <span>WEB LINK</span>
                    ...
                    <div className="py-4 border border-dashed border-white/5 rounded-lg bg-white/[0.01] flex flex-col items-center justify-center gap-3 text-text-dim/20">
                       ...
                       <p className="text-xs font-bold uppercase tracking-wide text-center">No deliverables yet</p>
                    </div>
                  )}
                </section>
                ...
                <section className="space-y-6">
                  <div className="flex items-center gap-3">
                    ...
                    <h4 className="text-xs font-bold text-text-dim/40 uppercase tracking-wide">Client uploads ({(item.assets ?? []).filter(a => a.role !== 'OUTPUT').length})</h4>
                  </div>
                  {(item.assets ?? []).filter(a => a.role !== 'OUTPUT').length > 0 ? (
                    <div className="grid grid-cols-2 gap-4">
                      {(item.assets ?? []).filter(a => a.role !== 'OUTPUT').map((asset) => (
                        <div key={asset._id} className="group/asset relative bg-black/40 border border-white/5 rounded-lg overflow-hidden hover:border-white/20 transition-all duration-500 shadow-xl">
                          <div className="aspect-[4/3] w-full bg-black/60 flex items-center justify-center relative overflow-hidden">
                            {asset.mimeType.startsWith('image/') ? (
                    ...
                    <div className="py-4 border border-dashed border-white/5 rounded-lg bg-white/[0.01] flex flex-col items-center justify-center gap-3 text-text-dim/20">
                       <p className="text-xs font-bold uppercase tracking-wide text-center">No client uploads</p>
                    </div>
                  )}
                </section>
              </div>
              ...
              <div className="space-y-6">
                <label className={cn(
                  ...
                  {uploading ? (
                    <div className="flex flex-col items-center gap-4">
                      ...
                      <span className="text-xs font-bold uppercase tracking-wide text-primary animate-pulse">Uploading to studio...</span>
                    </div>
                  ) : (
                    <>
                      ...
                      <div className="text-center space-y-1">
                        <p className="text-xs font-bold text-white uppercase tracking-wide">Add production deliverables</p>
                        <p className="text-xs font-medium text-text-dim/20 uppercase tracking-wide">Images, videos, or design assets</p>
                      </div>
                    </>
                  )}
                </label>
                ...
                <div className="bg-bg-card/40 border border-white/5 rounded-lg p-5 space-y-6">
                  <div className="flex items-center gap-3">
                    ...
                    <h4 className="text-xs font-bold text-text-dim/40 uppercase tracking-wide">External Link Deliverable</h4>
                  </div>
  ...
  if (isLoading) {
    return (
      <div className="flex flex-col items-center justify-center min-h-[60vh] gap-3 animate-in fade-in duration-700">
        ...
        <p className="text-xs font-bold uppercase tracking-wide text-text-dim/40 animate-pulse">Syncing production command center...</p>
      </div>
    )
  }
  ...
  return (
    <div className="max-w-5xl mx-auto p-4 md:p-4 space-y-5 pb-32 animate-in fade-in duration-1000">
      ...
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-3">
        <Link to="/admin/orders" className="group inline-flex items-center gap-4 text-text-dim/60 hover:text-white text-xs font-bold uppercase tracking-wide transition-all">
        ...
        <button onClick={fetchAll} className="flex items-center gap-3 text-text-dim/40 hover:text-primary text-xs font-bold uppercase tracking-wide transition-all bg-white/5 hover:bg-primary/10 px-4 py-3 rounded-lg border border-white/5 group">
        ...
        <div className="flex flex-col md:flex-row justify-between gap-3 relative z-10">
          <div className="space-y-4 flex-1">
            <div className="flex flex-wrap items-center gap-4">
              ...
              <StatusBadge status={order.status} className="text-xs px-4 py-1.5 font-bold uppercase tracking-wide rounded-lg shadow-xl" />
            </div>
            
            <div className="space-y-4">
               <h1 className="text-base md:text-base font-bold text-white tracking-tight  leading-tight">
                  {order.title.split(' ')[0]} <span className="text-primary ">{order.title.split(' ').slice(1).join(' ')}</span>
               </h1>
               <div className="flex flex-wrap items-center gap-3 text-text-dim/40">
                  <div className="flex items-center gap-2">
                     <Calendar size={14} className="text-primary/40" />
                     <p className="text-xs font-bold uppercase tracking-wide">Commissioned {new Date(order.createdAt).toLocaleDateString()}</p>
                  </div>
                  <div className="w-1.5 h-1.5 rounded-full bg-white/5" />
                  <div className="flex items-center gap-2">
                     <Package size={14} className="text-primary/40" />
                     <p className="text-xs font-bold uppercase tracking-wide">{items.length} Production units</p>
                  </div>
               </div>
            </div>
          </div>
          ...
          <div className="flex flex-col items-center md:items-end justify-center md:justify-start gap-4 text-center md:text-right bg-white/5 backdrop-blur-xl border border-white/5 p-4 rounded-lg shadow-lg min-w-[300px]">
             ...
             <p className="text-base font-bold text-primary  tracking-tight leading-none">{order.totalCreditsQuoted.toLocaleString()}</p>
             <div className="space-y-1">
                <p className="text-xs font-bold text-white/40 uppercase tracking-wide">Credits Captured</p>
                <div className="w-full h-1.5 bg-white/5 rounded-full overflow-hidden">
                   <div className="h-full bg-primary shadow-md transition-all duration-1000" style={{ width: `${(order.totalCreditsCaptured / order.totalCreditsQuoted) * 100}%` }} />
                </div>
                <p className="text-xs font-bold text-primary uppercase">{order.totalCreditsCaptured.toLocaleString()} CR</p>
             </div>
          </div>
        </div>

        <div className="grid grid-cols-1 md:grid-cols-2 gap-4 pt-12 mt-12 border-t border-white/5 relative z-10">
          <Link to={`/admin/users/${order.userId?._id}`} className="group/user flex items-center gap-3 p-5 bg-black/20 rounded-lg border border-white/5 hover:border-primary/20 transition-all duration-500 shadow-xl">
            <div className="relative">
               <img 
            ...
            <div>
              <p className="text-text-dim/20 text-xs uppercase tracking-wide font-bold mb-1">Client authority</p>
              <p className="text-white text-base font-bold tracking-tight group-hover/user:text-primary transition-colors">{order.userId?.name ?? 'Unknown Identity'}</p>
          ...
          <div className="flex flex-col justify-center p-5 bg-black/20 rounded-lg border border-white/5 shadow-xl relative group/assign">
            ...
            {assignError && <p className="text-error text-xs font-bold mt-2 uppercase tracking-wide">{assignError}</p>}
          </div>
        </div>
      </div>
        ...
        <div className="bg-primary/5 border border-primary/10 rounded-lg p-4 md:p-4 flex flex-col md:flex-row items-center justify-between gap-5 animate-in slide-in-from-top-4 duration-700 shadow-lg relative overflow-hidden group">
          ...
          <div className="space-y-3 relative z-10 text-center md:text-left">
            <div className="flex items-center justify-center md:justify-start gap-4">
               <div className="w-8 h-8 rounded-lg bg-primary flex items-center justify-center text-white shadow-xl shadow-primary/40 animate-pulse"><Clock size={24} /></div>
               <h2 className="text-base font-bold text-white tracking-tight ">Authorization <span className="text-primary ">pending</span></h2>
            </div>
          ...
          {reviewError && <p className="absolute bottom-4 left-1/2 -translate-x-1/2 text-error text-xs font-bold uppercase tracking-wide">{reviewError}</p>}
        </div>
      )}
      ...
      {order.status === 'IN_PROGRESS' && (
        <div className="bg-primary/5 border border-primary/10 rounded-lg p-4 md:p-4 flex flex-col md:flex-row items-center justify-between gap-5 animate-in slide-in-from-top-4 duration-700 shadow-lg relative overflow-hidden group mb-10">
          ...
          <div className="space-y-3 relative z-10 text-center md:text-left">
            <div className="flex items-center justify-center md:justify-start gap-4">
               <div className="w-8 h-8 rounded-lg bg-primary flex items-center justify-center text-white shadow-xl shadow-primary/40 animate-pulse"><UploadCloud size={24} /></div>
               <h2 className="text-base font-bold text-white tracking-tight ">Production <span className="text-primary ">active</span></h2>
            </div>
          ...
          <div className="bg-bg-card/40 backdrop-blur-3xl border border-white/5 rounded-lg overflow-hidden flex flex-col h-[850px] animate-in slide-in-from-right-12 duration-700 shadow-lg relative">
            ...
            <div className="bg-white/[0.02] px-4 py-8 border-b border-white/5 flex items-center justify-between z-10 backdrop-blur-xl">
              <div className="flex items-center gap-3">
                ...
                <div>
                  <h3 className="text-base font-bold text-white tracking-tight ">Studio <span className="text-primary ">connect</span></h3>
                  <div className="flex items-center gap-2 opacity-40">
                     <div className="w-1.5 h-1.5 rounded-full bg-success shadow-md animate-pulse" />
                     <p className="text-xs font-bold text-text-dim uppercase tracking-wide">End-to-End Encrypted Link</p>
                  </div>
                </div>
              </div>
            ...
            <div className="flex-1 overflow-y-auto custom-scrollbar p-4 space-y-6 scroll-smooth relative z-10">
              {messages.length === 0 ? (
                <div className="flex flex-col items-center justify-center h-full gap-4 opacity-5">
                  ...
                  <p className="text-base font-bold  tracking-tight uppercase">Silence established</p>
                </div>
              ) : (
                <div className="space-y-6">
            ...
            <form onSubmit={handleSendMessage} className="p-4 bg-bg-dark/40 backdrop-blur-3xl border-t border-white/5 z-20">
              ...
              <p className="text-center text-xs font-bold text-text-dim/10 uppercase tracking-wide mt-8">Studio Encrypted Terminal — Tactical Command Only</p>
            </form>
          </div>
        )}
          ...
          <div className="bg-bg-card/40 backdrop-blur-3xl border border-white/5 rounded-lg p-4 md:p-4 shadow-lg animate-in fade-in zoom-in-95 duration-1000 relative">
            ...
            <div className="max-w-4xl mx-auto relative z-10">
              <div className="flex items-center gap-5 mb-16 px-4">
                 ...
                 <div>
                    <h3 className="text-base font-bold text-white tracking-tight ">Audit <span className="text-primary ">history</span></h3>
                    <p className="text-xs font-bold text-text-dim/40 uppercase tracking-wide">Complete production lifecycle log</p>
                 </div>
              </div>
        ...
        <div className="fixed inset-0 z-[100] flex items-center justify-center p-4 md:p-4 animate-in fade-in duration-500">
          ...
          <div className="relative w-full max-w-5xl h-full flex flex-col gap-4 animate-in zoom-in-95 duration-700">
            <div className="flex-1 rounded-lg overflow-hidden border border-white/10 bg-black shadow-md relative flex items-center justify-center group/modal">
               ...
               <img src={resolveApiUrl(previewAsset.url)} alt={previewAsset.originalName} className="max-w-full max-h-full object-contain p-4 select-none drop-shadow-md transition-all duration-1000" />
               
               <div className="absolute top-10 right-10 flex gap-4">
                  <button onClick={() => setPreviewAsset(null)} className="w-8 h-8 bg-white/5 hover:bg-white text-white hover:text-black backdrop-blur-2xl rounded-lg flex items-center justify-center border border-white/10 transition-all hover:rotate-90 shadow-lg active:scale-90">
                     <X size={32} />
                  </button>
               </div>
            </div>
            ...
            <div className="bg-bg-card/60 backdrop-blur-3xl border border-white/10 p-4 md:p-4 rounded-lg flex flex-col md:flex-row justify-between items-center gap-3 shadow-lg relative overflow-hidden">
              ...
              <div className="flex items-center gap-5 relative z-10">
                ...
                <div className="space-y-2">
                  <h3 className="text-base md:text-base font-bold text-white  tracking-tight">{previewAsset.originalName}</h3>
                  <div className="flex flex-wrap items-center gap-4 text-xs font-bold uppercase tracking-wide text-text-dim/40">
                    <span className="bg-white/5 px-4 py-2 rounded-lg border border-white/5 shadow-xl">{(previewAsset.sizeBytes / 1024 / 1024).toFixed(2)} MB</span>
