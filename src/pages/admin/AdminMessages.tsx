      ...
      <div className="w-[320px] border-r border-white/5 bg-white/[0.01] flex flex-col h-full z-20">
        <div className="p-5 space-y-5">
          <div className="flex items-center justify-between">
            <div>
              <h2 className="text-lg font-black text-white tracking-tight">
                Studio <span className="text-primary italic">Connect</span>
              </h2>
              <p className="text-[8px] font-bold text-text-muted uppercase tracking-widest mt-0.5 opacity-40">
                Communications
              </p>
            </div>
          ...
          <div className="relative group">
            <Search
              className="absolute left-4 top-1/2 -translate-y-1/2 text-text-muted/20 group-focus-within:text-primary transition-all duration-300"
              size={12}
            />
            <input
              type="text"
            ...
            threads.map((thread) => (
              ...
              >
                <div className="relative flex-shrink-0">
                  <img
                    src={
                      resolveApiUrl(thread.user.avatar) ||
                  ...
                  <p
                    className={cn(
                      ...
                      thread.unreadCount > 0
                        ...
                        : 'text-text-muted opacity-40 group-hover:opacity-60'
                    )}
                  >
      ...
      <div className="flex-1 flex flex-col bg-bg-dark relative overflow-hidden h-full">
        {!activeUserId ? (
          <div className="flex-1 flex flex-col items-center justify-center p-12 relative z-10">
            ...
            <p className="text-[10px] font-bold text-white uppercase tracking-widest opacity-20">
              Select a Session
            </p>
          </div>
        ) : (
          <>
            <div className="h-16 border-b border-white/5 bg-bg-dark/60 backdrop-blur-3xl px-8 flex items-center justify-between z-30">
              <div className="flex items-center gap-3">
                ...
                <div>
                  ...
                  <p className="text-[7px] font-bold uppercase tracking-[0.2em] text-emerald-500 opacity-60">
                    Connected
                  </p>
                </div>
              </div>
              <div className="flex items-center gap-4">
                <span className="text-[8px] font-mono text-white/20">
                  ID-{activeUserId.slice(-8).toUpperCase()}
                </span>
            ...
            <div className="px-6 pb-6 pt-2 bg-bg-dark z-30">
              <form
                onSubmit={handleSend}
                className="max-w-4xl mx-auto flex items-center gap-3 bg-white/[0.02] border border-white/5 rounded-lg px-4 py-2 focus-within:border-primary/40 focus-within:bg-white/[0.03] transition-all"
              >
                <button
                  type="button"
                  className="text-text-muted/20 hover:text-primary transition-all"
                >
                  <Paperclip size={16} />
                </button>
                <input
              ...
              <div className="mt-3 flex items-center justify-center gap-2 opacity-10">
                ...
                <p className="text-[7px] font-bold text-text-muted uppercase tracking-[0.4em]">
